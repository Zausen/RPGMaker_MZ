...
/*:
 * @target MZ
 * @plugindesc (Optional) Extend and change the battle and menu skills informations. To use it, is necesary PaySystem plugin.
 * @author Fer Zacrón, test: Efímero
 *
 * @base PaySystem
 * @orderAfter PaySystem
 * 
 * @help PaySystemStyle.js
 *
 * This plugin complements the PaySystem plugin. This plugin changes the look 
 * of the skill selection in the battle menu and the main menu.
 *
 * @param Certain hit text
 * @desc Text for hit type certain.
 * @default Certain
 * 
 * @param Physical hit text
 * @desc Text for hit type physical.
 * @default Physical
 * 
 * @param Magical hit text
 * @desc Thext for hit type magical.
 * @default Magical
 * 
 * @param Element images
 * @desc Separated by commas and without spaces, the icon identifiers sorted by element.
 * @default 78,64,65,66,67,68,69,70,71
 * 
 * @param General payment text
 * @desc General text for the basic skill cost header from menu.
 * @default General cost: 
 * 
 * @param Necessary states text
 * @desc Text for the necessary states cost header from menu.
 * @default Necessary states: 
 * 
 * @param Necessary items text
 * @desc Text for the necessary items cost header from menu.
 * @default Necessary items:
 * 
 * @param Hit type text
 * @desc Text for hit type header from the menu. 
 * @default Hit type: 
 *
 * @param Update seconds Overlay
 * @desc It is the time between actor affected states over of the actor in the battle. (-1 or 0 for disable)
 * @default 3
 * 
 */
Zausen.FZ_SistemaCostesEstilo.PluginName = "PaySystemStyle";
...
Zausen.FZ_SistemaCostesEstilo.Parametros.TxtGolpeCertero = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Certain hit text'] || '');//1
Zausen.FZ_SistemaCostesEstilo.Parametros.TxtAtaqueFisico = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Physical hit text'] || '');//2
Zausen.FZ_SistemaCostesEstilo.Parametros.TxtAtaqueMagico = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Magical hit text'] || '');//3
Zausen.FZ_SistemaCostesEstilo.Parametros.IconosElementos = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Element images'] || '');//4
Zausen.FZ_SistemaCostesEstilo.Parametros.TextoCosteGeneral = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['General payment text'] || '');//5
Zausen.FZ_SistemaCostesEstilo.Parametros.TextoEstadosNecesarios = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Necessary states text'] || '');//6
Zausen.FZ_SistemaCostesEstilo.Parametros.TextoObjetosNecesarios = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Necessary items text'] || '');//7
Zausen.FZ_SistemaCostesEstilo.Parametros.TextoTipoHabilidad = String(Zausen.FZ_SistemaCostesEstilo.PluginManager['Hit type text'] || '');//8
Zausen.FZ_SistemaCostesEstilo.Parametros.VueltasActualizacionOverlay = Number(Zausen.FZ_SistemaCostesEstilo.PluginManager['Update seconds Overlay'] * 8 || -1);//8
