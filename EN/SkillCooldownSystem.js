...
/*:
 * @target MZ
 * @plugindesc Aggregate skill cooldown options and limit uses about skills while the battle.
 * @author Fer Zacrón, test: Efímero
 *
 * @help SistemaEnfriamientoHabilidades.js
 * 
 * 
 * For use it, the label is COOLDOWN{}
 * 
 * Between keys we can use the next sublabels:
 * TURNS:N (Where N is an integer for turns) 
 * TURNS:N-M (Where N and M are an integeres for turns. In practice, will be a random cooldown by use beetween N and M (includes))
 * USES:N (Where N is an integer for uses of skill by battle) 
 * 
 * Example 1:
 * Enfriamiento{
 *  TURNS:4
 * }
 * After use the skill, wont can use again for four turns.
 * Example 2:
 * COOLDOWN{
 *  TURNS:0-10
 * }
 * After use the skill, this could be inactive between 0 and 10 turns. (Random)
 * 
 * Example 3:
 * COOLDOWN{
 *  TURNS:1-2
 * }
 * After use the skill, Después de usar la habidliad, his could be inactive 1 or 2 turns. (Random)
 * 
 * Example 4:
 * COOLDOWN{
 *  USES:3
 * }
 * After use the skill 3 times, this disabler until end battle. 
 * 
 * TURNS and USES are compatibles.
 * 
 * 
 * The cooldown is for rounds and not for individual turns.
 *  * 
 */
Zausen.FZ_SistemaEnfriamientoHabilidades.Regex = {
    general : /COOLDOWN[{]([\s\S]*?)[}]/i,
    turnos: /TURNS[:](\d{0,2}[\-][1-9]\d{0,1}|[1-9]\d{0,1})/i,
    usos: /USES[:]([+\-]\d{1,2}|\d{1,2})/i
